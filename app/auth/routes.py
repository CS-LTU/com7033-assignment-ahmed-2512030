...
def verify_2fa():
    ...
    if form.validate_on_submit():
        ...
        otp_input = str(form.otp.data).strip()
        
        if totp.verify(otp_input, valid_window=4):
            login_user(user)
            session.pop('2fa_user_id', None)
            log_audit('login_2fa', {'username': user.username})
            return redirect(url_for('patients.index'))
        else:
            flash('Invalid OTP. Please check that your phone and computer clocks are synchronized.', 'danger')
...
def setup_2fa():
    ...
    if form.validate_on_submit():
        ...
        if totp.verify(otp_input, valid_window=4):
            ...
            log_audit('enable_2fa', {'username': current_user.username})
            return redirect(url_for('patients.index'))
        else:
            flash('Invalid OTP. Please ensure your phone and computer clocks are synchronized.', 'danger')
...
def disable_2fa():
    if current_user.is_2fa_enabled:
        ...
        log_audit('disable_2fa', {'username': current_user.username})
    return redirect(url_for('patients.index'))
